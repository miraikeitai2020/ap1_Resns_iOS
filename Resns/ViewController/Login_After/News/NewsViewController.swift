...
class NewsViewController: UIViewController, UIGestureRecognizerDelegate ,UICollectionViewDelegate{
    ...
    let CommentView = UIView.init(frame: CGRect.init(x: 53, y: 600, width: 315, height: 400))
    
    let commentCollectionView = UICollectionView( frame: CGRect(x: 53, y: 600, width: 315, height: 400),collectionViewLayout: UICollectionViewFlowLayout())
    
    override func viewDidLoad() {
        super.viewDidLoad()
        
        //presenter処理
        let articlePresenter = ArticlePresenter()
        //循環参照防止のweakself
        articlePresenter.application(completion: { [weak self] articleInfo in
            ...
            /*
            // コメント欄（仮）
            self.CommentTextView.backgroundColor = UIColor.red//UIColor(red: 1, green: 1, blue: 1, alpha: 1.0)
            // TextViewの編集
            ...
            //　フォント設定
            self.CommentTextView.font = UIFont(name:"Hiragino Kaku Gothic ProN",size:15)
            // 表示させるテキストを入れる場所
            self.CommentTextView.text = self.articleContext
            // 左詰めの設定をする.
            self.CommentTextView.textAlignment = NSTextAlignment.left
            self.CommentTextView.scrollRangeToVisible(NSMakeRange(1,10))
            */
            ...
            CommentView.layer.cornerRadius = 20.0   // 丸みのサイズを設定する
            CommentView.layer.borderWidth = 2       // 枠線の太さを設定する.
            CommentView.layer.borderColor = UIColor.gray.cgColor // 枠線の色をグレーに設定する.
            //self.view.addSubview(CommentView)    // OughtSideViewをViewに追加する.
            
            
            /*
            //コメント欄のつもりだったけど使わなそうだから後で消す
            //let CollectionView = UIView.init(frame: CGRect.init(x: 53, y: 600, width: 315, height: 400))
            commentCollectionView.backgroundColor = UIColor(red: 1, green: 1, blue: 1, alpha: 1.0)
            commentCollectionView.layer.masksToBounds = true  // 角に丸みをつける
            commentCollectionView.layer.cornerRadius = 20.0   // 丸みのサイズを設定する
            commentCollectionView.layer.borderWidth = 2       // 枠線の太さを設定する.
            commentCollectionView.layer.borderColor = UIColor.gray.cgColor // 枠線の色をグレーに設定する.
            */
        ...
        titleLabel3.text = "#吾峠呼世晴" // テキストの設定
        ...
        titleLabel4.text = "#週刊少年ジャンプ" // テキストの設定
        ...
        */
        ...
        /*
        let nib = UINib(nibName: "CommentCollectionViewCell", bundle: .main)
 ...
 */
    ...
    @objc func tappedShareButton(_ sender: UITapGestureRecognizer){
        ...
        //リリース後はアプリに飛ぶ
        let text2 = String("「鬼滅の刃」は、週刊少年ジャンプ(集英社)にて2016年11号から連載がスタートし2020年24号に終了した日本の漫画である。作者は...")
        let myWebsite = NSURL(string:"https://www.fun.ac.jp/")
        let HashTag = String("\n#Resns #アニメ・漫画 #鬼滅の刃 #吾峠呼世晴 #週刊少年ジャンプ\n")
        
        let shareItems = [ShareTitle, self.titleLabel.text, text2, myWebsite, HashTag] as [Any]
